...
!
      ...
      MESSAGE_CHECK = "Entered NEMS_INITIALIZE ....."
     CALL ESMF_LogWrite(MESSAGE_CHECK, ESMF_LOGMSG_INFO, rc = RC)
!
!-----------------------------------------------------------------------
...
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
      ...
      if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=259, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
!
!-----------------------------------------------------------------------
...
!
      CALL ESMF_GridCompSetInternalState(NEMS_GRID_COMP                 &  !<--The NEMS component
                                        ...
                                        ,RC)     
      if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=273, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
...
!
      CALL ESMF_VMGetGlobal(vm = VM_GLOBAL                              &  !<-- The ESMF global Virtual Machine
                           ,rc = RC)
      if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=282, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
      CALL ESMF_VMGet(vm       = VM_GLOBAL                              &  !<-- The ESMF global Virtual Machine
                     ...
                     ,rc       = RC)
      if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=288, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
...
!
      CALL ESMF_ConfigLoadFile(config   = CF_NEMS                       &  !<-- The configure object
                              ...
                              ,rc       = RC)
      if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=310, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!-----------------------------------------------------------------------
...
!
      CALL ESMF_ConfigGetAttribute(config = CF_NEMS                     &  !<-- The NEMS configure object
                                  ...
                                  ,rc     = RC)
      if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=325, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
!-----------------------------------------------------------------------
...
!
        CALL ESMF_ConfigGetAttribute(config=CF_NEMS                     &
                                    ...
                                    ,label ='nhours_fcst1:'             &
                                    ,rc    =RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=343, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
...
!
        CALL ESMF_TimeIntervalSet(timeinterval=RUNDURATION              &  !<-- The forecast length (s) (ESMF)
                                 ...
                                 ,rc          =RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=359, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
        CALL ESMF_ClockSet(clock       = CLOCK_NEMS                     &  !<-- The NEMS Clock
                          ,runDuration = RUNDURATION                    &  !<-- The forecast length (s) (ESMF)
                          ,rc          = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=364, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
      END IF
...
!
      CALL ESMF_ConfigGetAttribute(config = CF_NEMS                     &  !<-- The NEMS configure object
                                  ...
                                  ,rc     = RC)
      if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=383, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
!-----------------------------------------------------------------------
...
!
      ...
      DO I = 1, TOTAL_MEMBER
        ...
        CALL ESMF_ConfigGetAttribute(config = CF_NEMS                   &
                                    ...
                                    ,label  = TRIM(PELAB)               &
                                    ,rc     = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=414, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
...
!
        CALL ESMF_ConfigGetAttribute(config = CF_NEMS                   &
                                    ...
                                    ,label  = 'HH_INCREASE:'            &
                                    ,rc     = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=466, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
        CALL ESMF_ConfigGetAttribute(config = CF_NEMS                   &
                                    ,value  = hh_start                  &
                                    ,label  = 'HH_START:'               &
                                    ,rc     = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=472, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
        CALL ESMF_ConfigGetAttribute(config = CF_NEMS                   &
                                    ,value  = hh_final                  &
                                    ,label  = 'HH_FINAL:'               &
                                    ,rc     = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=478, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
...
!
      ...
      write(MESSAGE_CHECK, *) "Create EARTH grid Components for: ",I
      CALL ESMF_LogWrite(MESSAGE_CHECK,ESMF_LOGMSG_INFO,rc=RC)
        ...
        EARTH_GRID_COMP(i) = ESMF_GridCompCreate (                      &
                            ...
                            ,rc           = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=507, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return

        !! Beheen - at this time timeStep=runDuration=259200
        !call ESMF_ClockGet(CLOCK_NEMS, timeStep=timeStep, rc=rc)
        !call ESMF_TimeIntervalGet(timeStep, s=S, rc=rc)
        !write(MESSAGE_CHECK, *) "timeStep: ", S
        !call ESMF_LogWrite(MESSAGE_CHECK, ESMF_LOGMSG_INFO)

      END DO

      !! Beheen at this time timeStep=runDuration=259200
      !call ESMF_ClockGet(CLOCK_NEMS, timeStep=timeStep,name=EARTH_COMP_NAME(1), rc=rc)
      !call ESMF_TimeIntervalGet(timeStep, s=S, rc=rc)
      !write(MESSAGE_CHECK, *) "timeStep for component1 is: ", S
      !call ESMF_LogWrite(MESSAGE_CHECK, ESMF_LOGMSG_INFO)
...
!
      IF(ENS_SPS) THEN
        ENS_CPL_COMP=ESMF_CplCompCreate(name = "ENS Cpl component"      &  
                                       ,rc   = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=533, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END IF
!
!-----------------------------------------------------------------------
...
!
      DO I = 1, TOTAL_MEMBER
     write(MESSAGE_CHECK,*) "Register EARTH init for member: ", I
     CALL ESMF_LogWrite(MESSAGE_CHECK,ESMF_LOGMSG_INFO,rc=RC)
        ...
        CALL ESMF_GridCompSetServices(EARTH_GRID_COMP(I)                &  !<-- The EARTH gridded components
                                     ...
                                     ,rc=RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=553, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END DO
...
!
        CALL ESMF_CplCompSetServices(ENS_CPL_COMP                       &
                                    ...
                                    ,rc=RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=578, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
      END IF
!
!-----------------------------------------------------------------------
...
!
      DO I = 1,TOTAL_MEMBER
        EARTH_IMP_STATE(I) = ESMF_StateCreate(                          &
                                        ...
                                        ,rc        = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=596, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END DO
!
!-----------------------------------------------------------------------
!
...
!
      DO I = 1,TOTAL_MEMBER
        EARTH_EXP_STATE(I) = ESMF_StateCreate(                                 &
                                        ...
                                        ,rc          = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=611, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      END DO
!
!-----------------------------------------------------------------------
...
!
        ENS_CPL_IMP_STATE=ESMF_StateCreate(name   = "ENS_CPL_Import"         &
                                          ,stateintent = ESMF_STATEINTENT_IMPORT  &
                                          ,rc          = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=628, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
!-----------------------------------------------------------------------
!
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
...
!
        ENS_CPL_EXP_STATE=ESMF_StateCreate(name = "ENS_CPL_Export"           &
                                          ,stateintent = ESMF_STATEINTENT_EXPORT  &
                                          ,rc        = RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=640, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
!-----------------------------------------------------------------------
...
! 
        DO I = 1, TOTAL_MEMBER
          IF(MEMBER_ID == I) THEN
            ...
            if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=655, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
            CALL ESMF_StateAddReplace(ENS_CPL_EXP_STATE,(/EARTH_IMP_STATE(I)/), rc = RC)
            if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=657, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
          END IF
        END DO
!
      END IF
...
!-----------------------------------------------------------------------
      ...
      CALL ESMF_ConfigGetAttribute(config = CF_NEMS &
                                   ...
                                   ,default = 0 &
                                   ,rc     = RC)
      if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=676, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      if (fhrot > 0) then
        CALL ESMF_TimeIntervalSet(restartOffset, h=fhrot,rc=RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=679, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
        CURRTIME = STARTTIME + restartOffset
        call ESMF_ClockSet(CLOCK_NEMS, currTime=CURRTIME, rc=RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=682, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
      endif

      !! at this time timeStep=runDuration=259200
      !call ESMF_ClockGet(CLOCK_NEMS, timeStep=timeStep,name=EARTH_COMP_NAME(1), rc=rc)
      !call ESMF_TimeIntervalGet(timeStep, s=S, rc=rc)
      !write(MESSAGE_CHECK, *) "timeStep for component1 iiiiiiiiiis: ", S
      !call ESMF_LogWrite(MESSAGE_CHECK, ESMF_LOGMSG_INFO)
...
!
     ...
     write(MESSAGE_CHECK,*) "Execute the Initialize step for member: ", I
     CALL ESMF_LogWrite(MESSAGE_CHECK,ESMF_LOGMSG_INFO,rc=RC)
          ...
          CALL ESMF_GridCompInitialize(gridcomp    = EARTH_GRID_COMP(I)  &
                                      ...
                                      ,userRc      = RC_USER             &
                                      ,rc          = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=716, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
          if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=717, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
        END IF

        !! at this time timeStep=259200
        call ESMF_ClockGet(CLOCK_NEMS, timeStep=timeStep, rc=rc)
        call ESMF_TimeIntervalGet(timeStep, s=S, rc=rc)
        write(MESSAGE_CHECK, *) "NEMS_GRID_COMP timeStep is: ", S
        call ESMF_LogWrite(MESSAGE_CHECK, ESMF_LOGMSG_INFO)
!
      END DO
!
...
!
        CALL ESMF_CplCompInitialize(cplcomp    =ENS_CPL_COMP            &
                                   ...
                                   ,userRc     =RC_USER                 &
                                   ,rc         =RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=746, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
        if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=747, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_INIT)) return
!
      END IF
!
!-----------------------------------------------------------------------
!
      MESSAGE_CHECK = "Exited NEMS_INITIALIZE ....."
     CALL ESMF_LogWrite(MESSAGE_CHECK, ESMF_LOGMSG_INFO, rc = RC)
...
!
      RC_RUN = ESMF_SUCCESS
      MESSAGE_CHECK = "Entered NEMS_RUN ....."
     CALL ESMF_LogWrite(MESSAGE_CHECK, ESMF_LOGMSG_INFO, rc = RC)
!
!-----------------------------------------------------------------------
!***  For the moment, use a direct copy of the main Clock within
...
!
        IF(MEMBER_ID == I) THEN
          CALL ESMF_GridCompRun(gridcomp    = EARTH_GRID_COMP(I)        &
                               ...
                               ,userRc      = RC_USER                   &
                               ,rc          = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=827, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
          if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=828, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
        END IF
!
      END DO
!
...
!
          CALL ESMF_CplCompRun(cplcomp    =ENS_CPL_COMP                 &
                              ...
                              ,userRc     =RC_USER                      &
                              ,rc         =RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=855, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
          if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=856, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
          CALL ESMF_VMBarrier(vm = VM_GLOBAL, rc = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=859, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
!-----------------------------------------------------------------------
...
!
          CALL ESMF_ClockGet(clock       = CLOCK_NEMS                   &
                            ...
                            ,rc          = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=873, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
...
!
          CALL ESMF_ClockSet(clock    = CLOCK_NEMS                      &
                            ,currTime = CURRTIME                        &
                            ,rc       = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=880, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
...
!
          CALL ESMF_TimeIntervalGet(timeInterval = RUNDURATION          &
                                   ,h            = HH                   &
                                   ,rc           = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=890, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
          HH = HH + HH_INCREASE
!
          CALL ESMF_TimeIntervalSet(timeInterval = RUNDURATION          &
                                   ,h            = hh                   &
                                   ,rc           = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=897, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
          CALL ESMF_ClockSet(clock       = CLOCK_NEMS                   &
                            ,runDuration = RUNDURATION                  &
                            ,rc = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=902, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
!-----------------------------------------------------------------------
!***  Execute the Run step of each element in the EARTH component
!***  array.
...
!
            IF(MEMBER_ID == J) THEN
                CALL ESMF_GridCompRun(gridcomp    = EARTH_GRID_COMP(J)  &
                                     ...
                                     ,userRc      = RC_USER             &
                                     ,rc          = RC)
                if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=924, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
                if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=925, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
            END IF
!
          END DO
!
...
!
         CALL ESMF_ClockGet(clock       = CLOCK_NEMS                    &
                           ,runDuration = RUNDURATION                   &
                           ,rc          = RC)
         if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=940, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_RUN)) return
!
!-----------------------------------------------------------------------
!
      END IF sps
...
!
        IF(MEMBER_ID == I) THEN
          CALL ESMF_GridCompFinalize(gridcomp    = EARTH_GRID_COMP(I)   &
                                    ...
                                    ,userRc      = RC_USER              &
                                    ,rc          = RC)
          if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=1009, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_FINALIZE)) return
          if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=1010, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_FINALIZE)) return
        END IF
!
      END DO
!
...
!
        CALL ESMF_CplCompFinalize(cplcomp    =ENS_CPL_COMP              &
                                 ...
                                 ,userRc     =RC_USER                   &
                                 ,rc         =RC)
        if (ESMF_LogFoundError(RC, msg="Breaking out of subroutine", line=1031, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_FINALIZE)) return
        if (ESMF_LogFoundError(RC_USER, msg="Breaking out of subroutine", line=1032, file="module_NEMS_GRID_COMP.F90", rcToReturn=RC_FINALIZE)) return
!
      END IF
!
!-----------------------------------------------------------------------
